...
export default function App() {
  ...
  const [currentTab, setCurrentTab] = useState<'work' | 'assign' | 'warehouse' | 'export' | 'settings' | 'history'>('work');
      ...
      <SafeAreaProvider>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#18181b' }} edges={['top', 'bottom']}>
          ...
          <View className="flex-1 bg-zinc-50 relative">
            ...
            {currentTab === 'settings' && <SettingsScreen onLogout={handleLogout} onNavigateHistory={() => setCurrentTab('history')} />}
            {currentTab === 'history' && <HistoryScreen onBack={() => setCurrentTab('settings')} />}
          </View>
